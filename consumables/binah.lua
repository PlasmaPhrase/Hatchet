...
SMODS.Consumable {
    ...
    use = function(self, card, area, copier)
        ...
        if (to_big(#G.hand.highlighted) <= to_big(2) and not (to_big(#G.hand.highlighted) == to_big(1))) then
            ...
            for i = 1, #affected_cards do
                G.E_MANAGER:add_event(Event({
                    ...
                    delay = 0.1,
                    func = function()
                        affected_cards[i]:set_ability(G.P_CENTERS['m_hatchet_mercury'])            
                        return true
                    end
                }))
            end
    ...
    can_use = function(self, card)
        return ((to_big(#G.hand.highlighted) <= to_big(2) and not (to_big(#G.hand.highlighted) == to_big(1))))
    end
}